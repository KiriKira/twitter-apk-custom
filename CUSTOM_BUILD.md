...
Every custom patch build rewrites the APK version fields before signing: the Android `versionCode` is increased by `version_code_offset`, and the version name gains a `-piko.<offset>` suffix. This lets Android and Obtainium identify a rebuild of the same upstream X version as an update.
